...
export class ProcessorService {
            ...
            if (checkResult.rows.length === 0) {
              await client.query(
                `INSERT INTO did_jobs (order_id, did_job_id, status, model) 
                 VALUES ($1, $2, $3, $4)`,
                [orderId, tempGenerationId, 'pending', 'fal-ai/minimax/hailuo-2.3-fast']
              );
            }
          } finally {
            client.release();
      ...
      fakeProgressInterval = setInterval(async () => {
        try {
          ...
          if (elapsed < fakeProgressDuration) {
            currentFakeProgress = Math.min(70, Math.round((elapsed / fakeProgressDuration) * 70));
          } else if (elapsed < fakeProgressDuration + 30000) {
            const extraTime = elapsed - fakeProgressDuration;
            currentFakeProgress = 70 + Math.round((extraTime / 30000) * 15);
          } else if (elapsed < fakeProgressDuration + 60000) {
            const extraTime = elapsed - fakeProgressDuration - 30000;
            currentFakeProgress = 85 + Math.round((extraTime / 30000) * 10);
          } else {
  ...
  private async handleMultipleJobsSuccess(generationIds: string[], telegramId: number, orderId: string, videos: Array<{ url: string; model?: string }>): Promise<void> {
    try {
      ...
      // Атомарное обновление статуса (только если еще не completed) - защита от дублирования
      console.log(`   Обновляю статус заказа ${orderId} на 'completed' (атомарно)...`);
      const client = await (await import('../config/database')).default.connect();
      try {
        ...
        if (updateResult.rows.length === 0) {
          console.log(`⚠️ Заказ ${orderId} уже был завершен другим процессом, пропускаю отправку`);
          return;
        }
        
        console.log(`   ✅ Статус заказа ${orderId} обновлен на 'completed' (атомарно)`);
      } finally {
        client.release();
      }
    ...
    try {
      ...
      while (combineRetryCount <= maxCombineRetries) {
        try {
          ...
          // Если это некритичная ошибка таймаута, не бросаем ошибку и не отправляем сообщение пользователю
          // Вместо этого ждем и пробуем еще раз, или позволяем мониторингу подхватить результат
          if (isTimeoutError && error?.isNonCritical) {
            ...
            if (combineRetryCount < maxCombineRetries) {
              ...
              // Если попытки закончились, но операция может продолжаться в фоне,
              // не бросаем ошибку и не отправляем сообщение пользователю
              // Мониторинг подхватит результат, когда операция завершится
              console.log(`⚠️ Таймаут при объединении фото, но операция может продолжаться в фоне. Мониторинг подхватит результат.`);
              // Не отправляем ошибку пользователю, так как операция может завершиться успешно
              // Просто выходим из цикла - combinedImageUrl останется undefined, но это нормально
              // Мониторинг подхватит результат позже
              break; // Выходим из цикла, не бросая ошибку
            }
          } else if (isTimeoutError) {
            // Критичная ошибка таймаута
      ...
      // Если combinedImageUrl undefined, это может означать, что произошел некритичный таймаут
      // и операция продолжается в фоне - в этом случае не бросаем ошибку, мониторинг подхватит результат
      if (!combinedImageUrl) {
        // Проверяем, был ли это некритичный таймаут (операция может продолжаться в фоне)
        // Если да, не бросаем ошибку и не отправляем сообщение пользователю
        console.log(`⚠️ Не удалось получить результат объединения фото, но операция может продолжаться в фоне.`);
        console.log(`   Мониторинг подхватит результат, когда операция завершится.`);
        // Не бросаем ошибку - позволяем мониторингу подхватить результат позже
        return;
      }
      ...
      // Создаем временный generationId для немедленного запуска мониторинга
      const tempGenerationId = `fal_temp_${orderId}_${Date.now()}`;
      const generationIds = [tempGenerationId];
      ...
      // Запускаем мониторинг СРАЗУ с временным джобом
      console.log(`👀 Запускаю мониторинг СРАЗУ с временным джобом ${tempGenerationId}, вызов fal.ai выполняется асинхронно...`);
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            try {
              await client.query(
                `UPDATE did_jobs SET did_job_id = $1 WHERE did_job_id = $2 AND order_id = $3`,
                [systemRequestId, tempGenerationId, orderId]
              );
              console.log(`   ✅ Обновлен временный джоб ${tempGenerationId} на реальный ${systemRequestId}`);
            } finally {
              client.release();
            }
            ...
            console.error(`Error in async fal.ai call for combine_and_animate (attempt ${retryCount + 1}/${maxRetries + 1}):`, error);
            
            // Проверяем, является ли это ошибкой скачивания файла
            const isDownloadError = !error.isFileAccessError && error.message && (
              error.message.includes('Не удалось загрузить изображение') ||
              error.message.includes('Failed to download') ||
              error.message.includes('file_download_error')
