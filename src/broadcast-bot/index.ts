...
bot.start(async (ctx) => {
  ...
  await ctx.reply(
    ...
    '🌱 /check_organic - Проверить статус органических пользователей (исключая "unu")'
  );
});
...
bot.command('check_organic', async (ctx) => {
  ...
  await ctx.reply(
    ...
    'Это может занять некоторое время.\n\n' +
    '⚠️ Пользователи увидят индикатор "печатает..." на несколько секунд, но сообщения не получат.\n\n' +
    'ℹ️ Исключены пользователи из кампании "unu" (купленные пользователи).'
  );
...
bot.telegram.setMyCommands([
  ...
  { command: 'check_organic', description: 'Проверить статус органических пользователей (исключая "unu")' }
]);
